...
- **RECALL-2**：BM25 生产检索必须走 `search_with_plan`（minimum_should_match 语义），**禁止**回退到旧宽松 OR 的 `Bm25Index::search`（已删除）。
  - **🟠 P0-3 有界例外（2026-08-24 增补）**：多词 msm 严格检索**空结果**时，允许用宽松 OR（QueryParser）兜底重试一次——仅当严格检索一无所获才触发（`search_with_plan` 内实现，日志 `[bm25] msm 严格检索空结果 → 宽松 OR 兜底`）；不改变严格语义正常工作的查询，用于"类型名/专名无法凑够 msm 词数"的召回兜底。
- **RECALL-8（🟠 P0-1 增补）**：裸标识符（CamelCase/snake_case）**且**含代码定位词（定义/在哪/常量/结构体/字段/变量/枚举/函数/方法/实现/声明/源码/位置/代码/写法/签名）→ 直接路由 Code 意图（`is_code_query`），恢复符号路召回；定位词从紧防误伤文档类查询。
