...
**目标:** 为 Canvas 白板添加工具栏入口、节点拖拽、连接线箭头和边交互功能
...
| 文件         | 修改内容                                                            |
| ------------ | ------------------------------------------------------------------- |
| `index.html` | 主要修改文件，约 10360 行工具栏区域、17500-17600 行 canvas 渲染代码 |

---

## Phase 0: 工具栏入口与保存功能

### Task 0: 添加工具栏 Canvas 按钮

**Files:**

- Modify: `index.html:10360-10370` (工具栏按钮区域)
- Modify: `index.html:12443-12520` (`toggleFile` 函数)

- [ ] **Step 1: 在工具栏添加 Canvas 按钮**

在 dashboard-btn 后添加 canvas 按钮：

```html
<button
  id="canvas-btn"
  class="btn btn-info"
  onclick="toggleFile('canvas')"
  title="白板"
>
...
- [ ] **Step 2: 在 `toggleFile` 添加 'canvas' case**

在 `} else if (type === 'whiteboard') {` 后添加：

```javascript
} else if (type === 'canvas') {
    aiFabContainer.style.display = 'none';
    openCanvasFile();
}
```

- [ ] **Step 3: 添加 `openCanvasFile` 函数**
...
function openCanvasFile() {
  ...
  // 切换到 canvas 视图
  switchToView(contentMain);
  // 渲染空白 canvas
  renderCanvasFile(JSON.stringify(blankCanvas), contentMain);
  // 显示 canvas 保存按钮
  showCanvasControls();
  // 设置标志
  isCanvasFileDisplay = true;
  currentFileHandle = null;
}
```

- [ ] **Step 4: 添加 `showCanvasControls` 函数**

```javascript
function showCanvasControls() {
  // 隐藏其他控件
  hideAllTopRightControls();
  // 显示 canvas 专用控件
  const canvasControls = document.getElementById("canvas-top-right-controls");
  if (canvasControls) {
...
- [ ] **Step 5: 添加 canvas top-right-controls HTML**

在 `top-right-controls` div 后添加：

```html
<div
  id="canvas-top-right-controls"
  class="top-right-controls"
  style="display: none;"
>
  <div class="canvas-dropdown canvas-save-dropdown">
    <button id="canvas-save-toggle" class="btn btn-primary btn-sm">保存</button>
    <div id="canvas-save-menu" class="canvas-dropdown-menu canvas-save-menu">
      <div id="canvas-save-directory-tree" class="canvas-directory-tree"></div>
    </div>
  </div>
</div>
```

- [ ] **Step 6: 添加 `handleCanvasSaveToggleClick` 函数**

```javascript
async function handleCanvasSaveToggleClick(e) {
  e.stopPropagation();
  const saveMenu = document.getElementById("canvas-save-menu");
  if (saveMenu.classList.contains("canvas-open")) {
    saveMenu.classList.remove("canvas-open");
  } else {
    saveMenu.classList.add("canvas-open");
    await loadCanvasDirectoryTree();
  }
}
```

- [ ] **Step 7: 添加 `loadCanvasDirectoryTree` 函数**

```javascript
async function loadCanvasDirectoryTree() {
  const container = document.getElementById("canvas-save-directory-tree");
  ...
  await renderDirectoryTree({
    container: container,
    rootDirHandle: getRootHandle(),
    onSave: doSaveCanvasToFolder,
    getDefaultFileName: () => "untitled.canvas",
    getContent: () => content,
    closeMenu: () =>
      document
        .getElementById("canvas-save-menu")
        .classList.remove("canvas-open"),
  });
}
```

- [ ] **Step 8: 添加 `doSaveCanvasToFolder` 函数**

```javascript
async function doSaveCanvasToFolder(handle, relativePath) {
  const canvasContent = JSON.stringify(canvasData);
  await writeFileHandle(handle, canvasContent);
  currentFileHandle = handle;
  showNotification("✓ 保存成功", "success");
  document.getElementById("canvas-save-menu").classList.remove("canvas-open");
}
```
...
.canvas-save-menu.canvas-open {
  display: block;
}
.canvas-directory-tree {
  padding: 8px;
}
```
  ...
  const drag = d3
    ...
    .on("drag", function (event) {
      ...
      const currentY = parseFloat(
        d3.select(this).attr("transform").split(",")[1].replace(")", ""),
      );
