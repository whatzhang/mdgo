...
/// - 缓存 LanceStore 实例（Arc 共享），避免增量操作反复重建连接
/// - `indexing_lock` 防止并发 index_all 调用导致数据损坏
/// - 增量操作实时更新元数据文件
pub struct Indexer {
    config_store: Arc<ConfigStore>,
    /// 缓存的 LanceStore 实例（Arc 共享连接，按 dir_path 索引）
...
impl Indexer {
    ...
    /// 获取 Bm25Index 实例（每次新建，Tantivy 内部有 OS 级文件缓存）
    async fn get_bm25_index(&self, dir_path: &str) -> Result<Bm25Index, String> {
