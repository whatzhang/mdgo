...
> 本文件是 `outline-mindmap` 技能的**持久化格式硬规约**。前端 OPML 引擎
> （`main.html` 大纲视图 + jsMind 思维导图视图）按本规约解析与序列化，
> **AI 写出的 .opml 必须与本规约一致，否则前端打开会报错或丢属性**。
...
- **写**：一律写 `.opml`（本规约）。前端打开即大纲视图，一键切思维导图；另存 .mm 由前端完成。
- **读**：`.opml` 按本规约读；`.mm`（FreeMind）宽容读（`TEXT`/`FOLDED`/`POSITION` 等属性可忽略，只取标题与嵌套）；`.md` 按标题层级提取（`#`→层）。
