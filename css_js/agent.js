...
const ALLOWED_SKILL_TOOLS = ['kb_search', 'code_lookup', 'read', 'edit', 'delete', 'list_files', 'git_status', 'pomodoro'];
...
FrontendBridge.register('pomodoro', (action, args) => {
    ...
    switch (action) {
        case 'status': {
            ...
            const phase = running ? '进行中' : (st.timeLeft > 0 && st.timeLeft < st.totalTime) ? '已暂停' : '未开始';
