 ...
 * 【入口】main.html enterEditMode 调用 initSlidesButton()
 * 【依赖】运行时主脚本全局：MdgoDocument / markedParse / parseObsidianToHTML /
 *         showNotification
 * 【对外暴露】window.initSlidesButton / window.MdgoSlides
 */
(function () {
    'use strict';
    ...
    async function present(mdText) {
        ...
        overlay.innerHTML = `
            ...
            <div class="mdgo-slides-nav">
                ...
                <button type="button" class="mdgo-slides-prev">←</button>
                <button type="button" class="mdgo-slides-next">→</button>
                <button type="button" class="mdgo-slides-exit">退出(Esc)</button>
            </div>`;
        ...
        function close() {
            ...
            if (document.fullscreenElement === overlay) { try { document.exitFullscreen(); } catch (e) { } }
            overlay.remove();
        }
        function onKey(e) {
    ...
    window.initSlidesButton = function () {
        const footer = document.getElementById('editor-footer');
        if (!footer || footer.querySelector('.mdgo-slides-toggle')) return;
        const btn = document.createElement('button');
        btn.type = 'button';
        btn.className = 'mdgo-export-btn mdgo-slides-toggle';
        btn.textContent = '放映';
        btn.title = 'Markdown 幻灯片放映（--- 分页，←/→ 翻页，Esc 退出）';
        btn.addEventListener('click', async () => {
            const d = window.MdgoDocument;
            const text = d && d.getValue ? d.getValue() : '';
            if (!text.trim()) { notify('当前文档为空', 'warning'); return; }
            btn.disabled = true;
            try { await present(text); } finally { btn.disabled = false; }
        });
