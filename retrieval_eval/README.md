...
| 2026-08-24 | 基线 v3（RAG P0 全量修复 + docs 中文改名） | 0.599 | 0.474 | 0.443 | 4424ms | 1285 文件 / 12 万 chunk（token 预算分块）；截断 60 / 重切 3679；reranker 真实启用 |
| 2026-08-24 | **基线 v4（P0 精度修复）** | **0.671** | **0.514** | **0.492** | **3009ms** | P0-1 裸标识符路由 Code + P0-2 类型标注 const 符号提取 + P0-3 msm 空结果宽松兜底；q027/q028/q039 0→1.00；延迟 −32%（rerank 缓存命中） |

> **当前回归基线 = 基线 v4（2026-08-24）**。v3 延迟由 rerank 主导（占 90%+，每查询
> 2~7.7s）；MRR/NDCG 较 v2 下降有解释（真实精排 sigmoid 裁决 + 语料变化），
> 详见 `docs/检索评测与回归基线.md` §4 观察。
